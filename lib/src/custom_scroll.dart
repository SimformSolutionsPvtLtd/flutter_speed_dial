...
/// than a [CustomSingleChildScrollView] containing a [ListBody] or [Column] with
/// many children.
///
...
/// beyond the dimensions of the screen, then [CustomSingleChildScrollView] would be
...
class CustomSingleChildScrollView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // TODO(parth): Fetch this from latest...
...
class _RenderSingleChildViewport extends RenderBox
  ...
  double get _maxScrollExtent {
    ...
    if (axis == Axis.horizontal) {
      extent = child!.size.width - size.width;
    } else {
      extent = child!.size.height - size.height;
    }
  ...
  Offset _paintOffsetForPosition(double position) {
    ...
    } else if (axisDirection == AxisDirection.left) {
  ...
  RevealedOffset getOffsetToReveal(RenderObject target, double alignment, {
    ...
    // One-dimensional viewport has only one axis; override if provided.
    axis ??= this.axis;

    // Use the target's paint bounds if rect is not provided
    rect ??= target.paintBounds;

    // If target is not a RenderBox, return current offset and rect
    ...
    // Determine main axis extent and leading scroll offset based on axis direction
    ...
    // Calculate target offset and target rect
